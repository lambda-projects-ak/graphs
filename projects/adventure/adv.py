...
# def generate_direction(p):
#     random_number = random.random()
#     if p.currentRoom.n_to and 0 < random_number < .25:
#         return "n"
#     elif p.currentRoom.e_to and 0.25 < random_number < .50:
#         return "e"
#     elif p.currentRoom.s_to and 0.50 < random_number < .75:
#         return "s"
#     elif p.currentRoom.w_to and 0.75 < random_number < 1:
#         return "w"

def generate_direction(queue):
        if "n" in q:
            return "n"
        elif "e" in q:
            return "e"
...
def generate_opposite(direction):
    if direction == 'n':
        return 's'
    elif direction == 'e':
        return 'w'
...
# player traversal map
traversalPath = []
reverse_path = []

# create a graph with a dictionary of directions
player_map = {}

# assign current room to player_map dictionary
current_room = player.currentRoom.id
# save previous room for later use
previous_room = player.currentRoom.id

# start dictionary
player_map[player.currentRoom.id] = {}
# map exits to players current room
for exit in player.currentRoom.getExits():
    player_map[player.currentRoom.id][exit] = '?'

# perform logic to explore map while player has visited less rooms than the map contains

# print(player_map) => {0: {'n': '?', 's': '?', 'w': '?', 'e': '?'}}

# if the player map has less rooms than maze, keep exploring.
while len(player_map) < len(roomGraph):
    exits = player_map[player.currentRoom.id] # {'n': '?', 's': '?', 'w': '?', 'e': '?'}

    # use a DFS to get to a room with no unvisited exits
    q = []
    ...
    # if there are unexplored rooms, search for a room without an unexplored exit
    if len(q) > 0:
        generated_direction = generate_direction(q)

        # set previous room as current room, and move to next room in queue
        previous_room = player.currentRoom.id
        player.travel(generated_direction)
        # player.currentRoom.id = player.currentRoom.id

        # append directions
        traversalPath.append(generated_direction)
        # save every move in reverse_path to use with bfs to return to nearest room with an unexplored room
        reverse_path.append(generated_direction)

        # if current room has not been visited, add unexplored exits to the player_map
        if player.currentRoom.id not in player_map:
            room_exits = {}
            for exit in player.currentRoom.getExits():
                room_exits[exit] = '?'

            # update player_map graph with information gathered about new room exits
            # and link new room to previous rooms graph
            player_map[previous_room][generated_direction] = player.currentRoom.id
            room_exits[generate_opposite(generated_direction)] = previous_room
            player_map[player.currentRoom.id] = room_exits
        else:
            player_map[previous_room][generated_direction] = player.currentRoom.id
    else:
        path_back = []
        # move back through list of previously moved directions to get back to a room with unexplored exits
        for move in reverse_path[:: -1]:
            player.travel(generate_opposite(move))
            path_back.append(generate_opposite(move))
            reverse_path.pop(-1)
            if "?" in player_map[player.currentRoom.id].values():
...
# # Testing Random Player Movements
# test = {}
# moves = 0
...
#     test[current_room] = exits
#     move_random_direction(player)
#     moves += 1
# print(visited_rooms)
# print(test)
...
# add move to travelled path list
# once a room is reached with no unexplored rooms, do a bfs to the nearest room with unexplored directions
# pass in graph and current room player is in to find nearest path


# def bfs_to_nearest(rooms, starting_room):
#     # initialize empty queue
#     q = Queue()
#     # add path to current room
#     q.enqueue([starting_room])
#     # keep track of visited rooms
#     visited = set()

#     # while queue has pathes
#     while q.size() > 0:
#         # dequeue first path
#         path = q.dequeue()
#         # grab last visited room from path and search for unexplored exits
#         current_room = path[-1]
...
#             # then search for exits
#             for exit in graph[current_room]:
#                 if graph[current_room][exit] == "?":
#                     # when finding an unexplored room
#                     # return path to player to nearest unexplored exit
#                     return path


# # repeat until all there are no values in the graph with unvisited rooms
